...
var CuminData = {
	CreateInstance:function(nodeName,index=0,convertor=null){
		...
		if(handler.Root.IsSelect){
			...
			handler.Root.Root.addEventListener("change",function(){
				var value = handler.Root.GetInner();
				handler.Root.SetAttribute(handler.OutLinkValue,value);
        ...
        if (handler.Root.Root.Converts == null) {
			if(convertor==null){
				...
				handler.Root.Root.Converts = convertor;
			}
        ...
        handler.BindData = function (data, outData = null, to = null) {
            ...
            if (data instanceof Array) {
                if (this.IsSelect) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        this.NormalBind(data[i],i+1, outData, to);
                    }
                }
            } else {
                if (this.IsSelect) {
                    ...
                    this.NormalBind(data,1, outData, to);
                }
            }
            return this;
        }
		 ...
		 //绑定正常节点
        handler.NormalBind = function (data, index, outData = null, to = null) {
            var realData = this.Private_GetData(data);
            var cloneNode = this.Model.Clone();
            this.Private_SetNormalNode(cloneNode, realData, to);
            
			this.UpdateCurrentNode(data,index,outData,to + " [" + this.Flag + "=\"" + this.Index + "\"]",true);
        ...
        handler.Private_BindEvent = function (fNode,cNode) {
            if (cNode.IsRadioBox) {
                cNode.Changed(function (pNode) {
                    fNode.SetAttribute(handler.OutLinkValue, cNode.GetInner());
                });
            } else if (cNode.IsCheckBox) {
                cNode.Changed(function (pNode) {
                    if (pNode.Root.checked) {
                        var tempOutValue = fNode.GetAttribute(handler.OutLinkValue);
                        if (tempOutValue == null || tempOutValue == "undefined" || tempOutValue=="")
						{
							var result = cNode.GetInner();
                    ...
                    } else 
					{
						...
						tempOutValue = tempOutValue.replace(cNode.GetInner()+handler.SpliteChar, "")
			...
			operatorNodes.Foreach(function(index,tNode){
				if(tNode.GetInner()==name){
                    ...
                    tNode.AddClick(function () {
						...
						var tData = handler.GetJson(flag);
						handler.CurrentNode= handler.InstanceString+" ["+handler.Flag+"=\""+flag+"\"]";
		...
		handler.ClearCurrentNode=function(uniqueFlag=null){
			if(uniqueFlag==null){
				if(this.CurrentNode==null)
				{
					uniqueFlag = this.InstanceString+" [json]";
				}else{
					uniqueFlag = this.CurrentNode;
				}
			}
			var nodes = CuminMultiNode.CreateInstance(uniqueFlag);
			nodes.Foreach(function(i,dataNode){
				if(!dataNode.IsSelect && !dataNode.IsCheckBox && dataNode.Root.tagName!="TBODY" && !dataNode.IsRadioBox){
					dataNode.SetInner("");
				}
		...
		handler.DeleteCurrentNode=function(uniqueFlag=null){
			if(uniqueFlag==null){
				if(this.CurrentNode==null)
				{
					uniqueFlag = this.InstanceString+" [json]";
			...
			CuminSingleNode.CreateInstance(uniqueFlag).Delete();
			this.CurrentNode = null;
		}
		
		
		handler.UpdateCurrentNode=function(data, index, outData,uniqueFlag=null,isClone=false){
			...
			var recoder = uniqueFlag+" [" + this.Content + "]";                                //Show:"#Dialog [CuminFlag=1] [field]"
			var outlink = uniqueFlag+" [" + this.OutLinkValue + "]";    					   //Show:"#Dialog [CuminFlag=1] [out]"
            var remove = uniqueFlag+" [" + this.OutLinkValue + "] [" + this.Content + "]";     //Show:"#Dialog [CuminFlag=1] [out] [field]"
			var sort = uniqueFlag+" ["+this.SortNumber+"]";
			...
			var needSort = CuminSingleNode.CreateInstance(sort);
			if(needSort!=null){
				needSort.SetInner(index);
			}
            ...
            if (outData!=null){
                outNodes.Foreach(function (index, node) {
					...
					if(outData.hasOwnProperty(key)){
						CuminData.CreateInstance(uniqueFlag+" ["+handler.Content+"=\""+key+"\"]",handler.Index,handler.Root.Root.Converts).BindData(outData[key], outData);
					}
                });
            }
			this.CurrentNode = null;
...
function InitTemplate(){
	CuminMultiNode.CreateInstance("[template]").Foreach(function(index,node){
		node.SetDisplay("none");
	});
	
}

function ClearCuminData(nodeName){
...
function GetNodeJson(nodeName){
	...
	outNodes.Foreach(function(i,dataNode){
		var key = dataNode.GetAttribute("json");
		if(key==null || key=="undefined" || key==""){
			key = dataNode.GetAttribute("field");
		}	
